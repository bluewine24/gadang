...
| F120 | 코스 조건 입력 (기본 → 확장) | ✅ 완료 |
| F121 | 비용 초과 알림 + 무료 장소 우선 | ✅ 완료 |
| F122 | 시간 기반 방문 가능 판단 | ✅ 완료 |
| F123 | ODsay 교통 경로·요금 (도시내/도시간) | ✅ 완료 |
| F124 | 총 예상 비용 계산 | ✅ 완료 (지역 카드 요금 표시) |
| F125 | 코스 자동 추천 (기본 → 고도화 → 3종) | ✅ 완료 |
| F126 | 여행 일정 저장 | ✅ 완료 |
| F127 | 일정 수정·재계산 | ✅ 완료 |
| F128 | 대체 장소 추천 | ✅ 완료 |
| F132 | 공유 코스 가져오기 + 재계산 | ✅ 완료 |
...
| 코스 조건 입력 | 생성 결과 타임라인 |
|---|---|
| ![코스 조건 입력 화면](../images/screen_course.png) | ![코스 생성 결과 화면](../images/screen_itinerary.png) |
...
**결정 사항**
- API 키는 환경변수로 주입하고 `.env.example`로 필요한 키 목록만 공유
- 버스 Odsay 실패 시 `BusFrequencyTable` 정적 fallback → `-1` 대신 실측값 제공
- 무궁화 하루 편수: trainServiceTime 값의 2/3 추정 (KTX 포함 전체 편수 기준)

**완료 후 점검 항목**
- ODsay 역명 매칭과 fallback 동작 확인
- 코스 추천·저장·수정·대체 장소·공유 코스 재계산 흐름 검증
- 지도 페이지의 허브 기준 장소 탐색 연동 확인
