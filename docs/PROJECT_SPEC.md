...
> 참고: SSAFY GitLab README에서 Mermaid가 정상 표시되지 않을 수 있으므로, 본 문서는 **Mermaid 없이 표와 텍스트 다이어그램**으로 작성한다.
...
| 코스 조건 입력 | 여행 일정 타임라인 |
|---|---|
| ![코스 조건 입력 화면](../images/screen_course.png) | ![여행 일정 화면](../images/screen_itinerary.png) |
...
![커뮤니티 화면](../images/screen_community.png)
...
![마이페이지 화면](../images/screen_mypage.png)
...
![유스케이스 관계도](../images/enjoytrip_usecase_diagram.png)
...
| AI Backend  | Spring AI 2.0.0, GMS(OpenAI 호환 API), Tool Calling                  |
| RAG Server  | FastAPI, Uvicorn, NumPy, `text-embedding-3-small`                    |
| Database    | MySQL 8 (`gadang` DB, user: `ssafy`, port: 3306)                     |
